...
async function showAllPositions() {
  ...
  try {
    ...
    // Get total supply of position NFTs from positionFactory
    ...
    console.log(`🏷️  Position NFT Address: ${positionNFTAddress}`);

    // We'll check for positions by trying to query the positionManager directly
    // Since we don't have easy access to totalSupply, we'll check a reasonable range
    const maxCheckRange = 100; // Check up to 100 position IDs
    
    console.log(`� Checking for positions (up to ${maxCheckRange} IDs)...`);
    ...
    for (let i = 1; i <= maxCheckRange; i++) {
      try {
        // Try to get position details - this will fail if position doesn't exist
        ...
        // Token might not exist or not owned by us, continue
        continue;
      }
    }

    if (positions.length === 0) {
      console.log('📭 No positions owned by this account');
      return;
    }

    console.log(`\n📈 Positions Summary (${positions.length} positions):`);
